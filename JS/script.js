...
elForm.addEventListener("submit", e =>{
        ...
        let {search, genre , sortBy} 
        =e.target.elements
        
        let regex = new RegExp(search.value.trim(), 
        "gi");
        
        let poiscFilms = films.filter((film) => 
        film.Title.match(regex) );
        
        
        if(genre.value !="all"){
                let filtrPoiscByGenre = 
                poiscFilms.filter((film) => 
...
function renderGenrel(array, element){
        ...
        array.forEach((film) =>{
                film.genres.forEach(genre =>{   
                        !generArray.includes(genre) ? 
                        generArray.push(genre) : null ;
                })
        });
